...
def correlation_model(df: pd.DataFrame, threshold: float):
    """Threshold is for the minimum correlation acceptable for an edge."""
...
def aracne_model(df: pd.DataFrame, discr_bins: int, threshold: float, remove=True):
    ...
    def rel_frequencies_pair(gene_array1: np.array, gene_array2: np.array, values: np.array):
        ...
        for i, value1 in enumerate(values):
            for j, value2 in enumerate(values):
                result[i,j] = np.count_nonzero((gene_array1==value1) & (gene_array2==value2))
    ...
    for i in range(5):
        for j in range(i+1,5):
            p_x = rel_frequencies_single(data_disc[:,i], disc_values)[:,np.newaxis]
            p_y = rel_frequencies_single(data_disc[:,j], disc_values)[:,np.newaxis]
            p_xy = rel_frequencies_pair(data_disc[:,i], data_disc[:,j], disc_values)
            mi = 0
